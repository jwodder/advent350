...
course, that not all of the letters are upper-case.  As in most ports, the
"magic" features of the original (i.e., forced delays before restarting a saved
game and "prime time" hours during which the game cannot be normally played)
are omitted, but this copy should have come with an alternative version of the
game, B<advmagic>, which retains these features.
...
Save files are completely vulnerable to editing by malicious players -- but
you'd have to be pretty pathetic to want to edit them dishonestly in the first
place.
